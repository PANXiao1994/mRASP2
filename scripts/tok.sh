...
TOKENIZERS_SCRIPTS="$( cd "$( dirname "$0" )" && pwd )"/tokenizers
...
# Chinese
if [ "$lg" = "zh" ]; then
  cat - | $REPLACE_UNICODE_PUNCT | $NORM_PUNC -l $lg | $REM_NON_PRINT_CHAR | python3 $CHINESE_TOKENIZER
# Thai
elif [ "$lg" = "th" ]; then
  cat - | python3 $THAI_TOKENIZER
# Japanese
elif [ "$lg" = "ja" ]; then
...
# Burmese
  ...
  cat - | python3 ${MY_SEGMENT}
# Arabic
elif [ "$lg" = "ar" ]; then
  cat - | ${AR_TOKENIZER}
# Indic
elif [ "$lg" = "ne" ]; then
  cat - | python3 ${IN_TOKENIZER} $lg
elif [ "$lg" = "si" ]; then
  cat - | python3 ${IN_TOKENIZER} $lg
elif [ "$lg" = "hi" ]; then
  cat - | python3 ${IN_TOKENIZER} $lg

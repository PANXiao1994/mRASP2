...
if [[ ! -z ${meta_mono_data} ]]; then
  lang_str=""
  IFS=',' read -r -a lang_list <<< ${langs}
  for lang in "${lang_list[@]}"
  do
      if [[ -d ${meta_mono_data}/${lang} ]]; then
          echo "${lang}"
          lang_str=${lang_str}",${lang}-${lang}"
          cp ${meta_mono_data}/${lang}/dict.* ${data_dir}/ &
      fi
  done
fi
...
fairseq-train ${data_dir} \
  ...
  --save-dir ${model_dir} \
  ${options} \
  --extra-data {\"${meta_mono_key}\":\"${meta_mono_data}\"} \
  --extra-lang-pairs {\"${meta_mono_key}\":\"${lang_str:1}\"} \

...
from . import SampledMultiEpochDatasetNew
from .. import StrictResamplingDataset, LanguagePairDatasetWithLangtoks
...
class MultilingualDatasetManager(object):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--collate-encoder-langtok",
            default=False,
            action="store_true",
            help="collate encoder langtok, if set True, please first ensure that --encoder-langtok=src",
        )
        parser.add_argument(
        ...
        parser.add_argument('--subsample-step-size', default=1, type=int, help='the subsample step size')
        parser.add_argument('--size-ratio', default=1.0, type=float, help='the ratio to resample training data')
        parser.add_argument('--step-size', default=2, type=int, help='step size')
        # parser.add_argument('--min-step-size', default=10, type=int, help='min step size')
        parser.add_argument(
            "--virtual-epoch-size",
            default=100000000,
            type=int,
    ...
    def prepare(cls, load_dictionary, args, **kargs):
        ...
        if args.collate_encoder_langtok:
            assert args.encoder_langtok == "src", ("if you set --collate-encoder-langtok, please first ensure that --encoder-langtok=src")
        ...
        for k in itertools.count():
            ...
            if training:
                if split == self.args.train_subset:
                    if self.args.subsample_step_size is not None and self.args.subsample_step_size > 1:
                        src_dataset = StrictResamplingDataset(src_dataset, step_size=self.args.subsample_step_size,
                                                              batch_by_size=False, epoch=0, seed=0)
                        tgt_dataset = StrictResamplingDataset(tgt_dataset, step_size=self.args.subsample_step_size,
                                                              batch_by_size=False, epoch=0, seed=0)
                    src_dataset = StrictResamplingDataset(src_dataset, step_size=self.args.step_size, batch_by_size=False,
                                                          epoch=epoch, seed=self.args.seed)
                    tgt_dataset = StrictResamplingDataset(tgt_dataset, step_size=self.args.step_size, batch_by_size=False,
                                                          epoch=epoch, seed=self.args.seed)
                elif split == self.args.valid_subset:
                    src_dataset = StrictResamplingDataset(src_dataset, step_size=10,
                                                          batch_by_size=False, epoch=0, seed=0)
                    tgt_dataset = StrictResamplingDataset(tgt_dataset, step_size=10,
                                                          batch_by_size=False, epoch=0, seed=0)
    ...
    def load_langpair_dataset(
        ...
        if self.args.collate_encoder_langtok:
            ds = LanguagePairDatasetWithLangtoks(
                src_dataset,
                src_dataset.sizes,
                src_dict,
                tgt_dataset,
    ...
    def load_a_dataset(
        ...
        if self.args.lang_tok_replacing_bos_eos:
            ...
            ds = langpair_ds
        # ds = ResetLanguagePairDataset(ds, epoch)
        return ds
        ...
        datasets = [
            (
                ...
                self.load_a_dataset(
                    ...
                    langpairs_sharing_datasets=langpairs_sharing_datasets,
                    training=training,
                    prepend_bos=kwargs["prepend_bos"],
                    **param,
                ),
            )
            for param in data_param_list
    ...
    def load_sampled_multi_epoch_dataset(
        ...
        if training and split == getattr(self.args, "train_subset", None):
            ...
            return SampledMultiEpochDatasetNew(
                OrderedDict(datasets),
                epoch=epoch,

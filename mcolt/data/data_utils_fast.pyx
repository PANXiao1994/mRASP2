...
@cython.cdivision(True)
cpdef np.ndarray[DTYPE_t, ndim=2] batch_by_size_fast_return_index(
    np.ndarray[DTYPE_t, ndim=1] indices,
    num_tokens_fn,
    long max_tokens,
    long max_sentences,
    ...
    for i in range(len(indices_view)):
        ...
        if _is_batch_full(batch, num_tokens, max_tokens, max_sentences):
            ...
            # batches.append(batch[:mod_len])
            # mod_len = mod_len - 1 # do not maintain the last sentence, which easily causes error when gpu memory is small
            batches.append([batch[0], batch[mod_len-1]])
            batch = batch[mod_len:]
            sample_lens = sample_lens[mod_len:]
            sample_len = max(sample_lens) if len(sample_lens) > 0 else 0
        batch.append(i)

...
class ThroughputTimer():
    ...
    def stop(self, report_speed=True):
        ...
        if self.total_step_count > self.start_step:
            ...
            if self.local_step_count % self.steps_per_output == 0:
                if report_speed:
                    self.logging("{}/{}, throughput={:.2f} tokens/s, timer per step={:.2f} seconds, time per epoch={:.2f} seconds".format(
                        self.epoch_count,
                        self.local_step_count,
                        self.avg_tokens_per_sec(),
                        self.avg_seconds_per_step(),
                        self.avg_seconds_per_epoch()))

    def avg_tokens_per_sec(self):
        if self.total_step_count > 0:
            tokens_per_step = self.tokens * self.num_workers
    ...
    def avg_seconds_per_epoch(self):
        if self.total_step_count > 0:
            total_step_offset = self.total_step_count - self.start_step
            avg_time_per_step = self.total_elapsed_time / total_step_offset
            return avg_time_per_step * self.epoch_size
        return float("-inf")
